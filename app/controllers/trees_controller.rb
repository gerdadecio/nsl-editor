...
class TreesController < ApplicationController
  ...
  def update_comment
    ...
    profile = Tree::Workspace::Profile.new(
      username: current_user.username,
  ...
  def update_tree_parent
    ...
    reparent = Tree::Workspace::Reparent.new(
      username: current_user.username,
      target: target,
      parent: parent,
    )
  ...
  def run_diff
    ...
    @result = RestClient.get(url, { content_type: :html, accept: :html })
    return unless /Nothing to see here.*no changes, nothing, zip/.match?(@result)

    @result = @result.sub(
      %r{<h3>Nothing to see here.</h3> *<p>We have no changes, nothing, zip.</p>},
      "<h4>No changes.</h4>",
    )
  rescue RestClient::Unauthorized, RestClient::Forbidden, RestClient::ExceptionWithResponse => e
    @message = json_error(e)
