...
class Loader::NamesController < ApplicationController
  ...
  def new
    @anchor = Loader::Name.find(params[:loader_name_id]) if params[:loader_name_id].present?
    ...
    @loader_name.seq = @anchor.seq + 1 if @anchor.present?
    @loader_name.family = @anchor.family if @anchor.present?
  ...
  def update
    ...
    @message = @loader_name.update_if_changed(
      loader_name_params
            .reject { |p| p.match(/parent_typeahead/) },
  ...
  def create_when_loaded_from_existing_instance
    ActiveRecord::Base.transaction do
      ...
      if params["form-task"] == "supplement-existing-concept"
        @loader_name.create_flipped_synonym_for_instance(loader_name_params, @current_user)
      end
      if loader_name_params["add_sibling_synonyms"] == "true"
        @loader_name.create_sibling_synonyms_for_instance(loader_name_params["loaded_from_instance_id"], @current_user)
      end
      if loader_name_params["add_sourced_synonyms"] == "true"
        @loader_name.create_sourced_synonyms_for_instance(
          loader_name_params[:loaded_from_instance_id],
          loader_name_params[:remark_to_reviewers],
          @current_user,
        )
      end
    end
