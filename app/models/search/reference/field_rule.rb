...
reference child where child.parent_id = reference.id) ",
    ...
    "is-not-a-parent:" => {
      where_clause: " not exists (select null from
reference child where child.parent_id = reference.id) ",
      takes_no_arg: true,
    },
    "has-no-children:" => {
...
reference child where child.parent_id = reference.id)",
    ...
    "is-not-published:" => {
      where_clause: " not published",
      takes_no_arg: true,
    },

    "author-exact:" => {
      where_clause: " author_id in (select id from
...
comment where comment.reference_id = reference.id and comment.created_by
    ...
    "publication-date:" => {
      where_clause: " lower(publication_date)
                                 like lower(?)",
    },
...
in (select id from ref_type where lower(name) like lower(?)) )",
    ...
    "not-type:" => {
      where_clause: " ref_type_id not in (select id
from ref_type where lower(name) like lower(?))",
    },

    "author-role:" => {
      where_clause: " ref_author_role_id in
(select id from ref_author_role where lower(name) like lower(?))",
    ...
    "published-location:" => {
      where_clause: " lower(published_location)
                                 like lower(?)",
    },
    "publisher:" => {
      where_clause:
    ...
    "volume:" => {
      where_clause: " lower(volume) like lower(?)",
    ...
    "iso-pub-date-matches:" => {
      where_clause: " iso_publication_date ~ ? ",
    ...
    "parent-id-sort-by-volume:" => {
      where_clause: " id = ? or parent_id = ?",
      order: "case when parent_id is null then
                                 'A' else 'B' end, volume, citation",
    },

    "parent-id-sort-numerical-by-volume:" => {
      where_clause: " id = ? or parent_id = ?",
      order: "case when parent_id is null then
...
(select id from author where lower(name) like lower(?))",
    ...
    "parent-ref-wrong-child-type:" => {
      where_clause: "reference.id in (
...
inner join ref_type xcrt on xrt.id = xcrt.parent_id))",
    ...
    "no-year:" => {
      where_clause: " iso_publication_date is null ",
      takes_no_arg: true,
    },
    "pub-date-is-year:" => {
    ...
    "has-no-direct-or-child-instances:" => {
      where_clause: " reference.id not in
        (select id
           from reference ref1
      where exists (
          select null
