...
#
#   Identify an author entered into or selected into a reference typeahead.
class Reference::AsResolvedTypeahead::ForAuthor
  include Resolvable

  attr_reader :value

  def initialize(id_string, param_text, field_name = "Author")
  ...
  def text_only
    possibles = ::Author.lower_name_equals(@text)
    case possibles.size
    when 0
  ...
  def zero_possibles
    possibles = ::Author.lower_name_like(@text + "%")
    case possibles.size
    when 1
      @value = possibles.first.id
    else
