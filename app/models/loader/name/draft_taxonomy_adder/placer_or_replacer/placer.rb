...
class Loader::Name::DraftTaxonomyAdder::PlacerOrReplacer::Placer
  ...
  def name_parent_tve
    ...
    raise "Error identifying name parent in draft: #{e}"
  end
  ...
  def intended_parent_tve
    ...
    raise "Error identifying intended parent in draft: #{e}"
  end
  ...
  def debug(msg)
    Rails.logger.debug { "Loader::Name::DraftTaxonomyAdder::PlaceOrReplace::Placer: #{msg}" }
  end
