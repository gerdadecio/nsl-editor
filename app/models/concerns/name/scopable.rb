...
module Name::Scopable
  ...
  included do
    ...
    scope :full_name_like,
      (lambda do |string|
         where(
           "lower(f_unaccent(full_name)) like lower(f_unaccent(?)) ",
           string.tr("*", "%") + "%",
         )
       end)
    scope :case_sensitive_full_name_like,
      (lambda do |string|
         where(
           "f_unaccent(full_name) like f_unaccent(?) ",
           string.tr("*", "%"),
         )
       end)
    ...
    scope :from_a_higher_rank,
      (lambda do |rank_id|
         joins(:name_rank).where(
           "not name_rank.deprecated and
                                        name_rank.sort_order < (select sort_order
                                        from name_rank where id = ?)",
           rank_id,
         )
       end)
    ...
    scope :but_rank_not_too_high,
      (lambda do |rank_id|
         where(
           "name_rank.id in (select id from name_rank where sort_order
    ...
    scope :avoids_id, ->(avoid_id) { where.not(name: { id: avoid_id }) }
    scope :all_children,
      (lambda do |parent_id|
         where(
           "name.parent_id = ? or name.second_parent_id = ?",
           parent_id,
           parent_id,
         )
       end)
