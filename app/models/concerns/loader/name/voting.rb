...
module Loader::Name::Voting
  ...
  def summary_of_votes_for_review(review)
    name_review_votes.includes([:org]).where(batch_review_id: review.id)
  ...
  def frequencies_of_votes_for_review(review)
    freq = Hash.new(0)
    name_review_votes.where(batch_review_id: review.id).pluck(:vote).map { |vote| vote ? "agree" : "disagree" }.each { |x| freq[x] += 1 }
