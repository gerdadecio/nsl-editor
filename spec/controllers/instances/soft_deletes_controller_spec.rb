...
RSpec.describe Instances::SoftDeletesController, type: :controller do
  ...
  describe "POST #create" do
    let(:allowed_result) do
      double("CheckDeleteResult", soft_delete_allowed?: true,
        explanation: "used only in past tree versions")
    end
    let(:blocked_result) do
      double("CheckDeleteResult", soft_delete_allowed?: false,
        explanation: "Instance is referenced by a live record")
    end

    context "when soft delete is allowed" do
      before do
        allow_any_instance_of(Instance).to receive(:check_delete_result).and_return(allowed_result)
      end
    ...
    context "when the model validation does not allow the soft delete" do
      before do
        allow_any_instance_of(Instance).to receive(:check_delete_result).and_return(blocked_result)
      end
      ...
      it "does not set deleted_at and renders the error template with the reason" do
        ...
        expect(assigns(:message)).to eq("Soft delete not allowed: Instance is referenced by a live record")
