...
# line when @reference.api_at is set.
class ReferenceShowEditorDetailsTabShowsBulkChangedWhenApiAtPresentTest < ActionController::TestCase
  ...
  test "shows a Bulk changed line with api_name and api_at" do
    ...
    expected = "by sample-api-user #{I18n.l(@reference.reload.api_at, format: :default)}"
    assert_match(/#{Regexp.escape(expected)}/, response.body)
  end

  test "shows unknown in the Bulk changed line when api_name is blank" do
    @reference.update_columns(api_name: nil, api_at: @api_at)
    show_details_tab
    expected = "by unknown #{I18n.l(@reference.reload.api_at, format: :default)}"
    assert_match(/#{Regexp.escape(expected)}/, response.body)
  end
  ...
  test "escapes html in api_name" do
    @reference.update_columns(api_name: "<b>bad</b>", api_at: @api_at)
    show_details_tab
