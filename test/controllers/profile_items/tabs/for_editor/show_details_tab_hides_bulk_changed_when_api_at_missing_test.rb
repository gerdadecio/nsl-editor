...
# @profile_item.api_at is blank - even if api_name is set.
class ProfileItemsTabsForEditorShowDetailsTabHidesBulkChangedWhenApiAtMissingTest < ActionController::TestCase
  ...
  test "hides the Bulk changed line when api_name and api_at are blank" do
    @profile_item.update_columns(api_name: nil, api_at: nil)
    show_details_tab
    assert_no_match(/Bulk changed/, response.body)
  end

  test "hides the Bulk changed line when only api_name is set" do
    @profile_item.update_columns(api_name: "sample-api-user", api_at: nil)
    show_details_tab
    assert_no_match(/Bulk changed/, response.body)
