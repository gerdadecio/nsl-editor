...
class InstanceTabsNotesShowsBulkChangedWhenApiAtPresentTest < ActionController::TestCase
  ...
  test "shows a Bulk changed line with api_name and api_at" do
    @instance_note.update_columns(api_name: "sample-api-user", api_at: @api_at)
    show_notes_tab
    assert_match(/Bulk changed/, response.body)
    assert_match(/2 days&nbsp;ago/, response.body)
    expected = "by sample-api-user #{I18n.l(@instance_note.reload.api_at, format: :default)}"
    assert_match(/#{Regexp.escape(expected)}/, response.body)
  end

  test "shows unknown in the Bulk changed line when api_name is blank" do
    @instance_note.update_columns(api_name: nil, api_at: @api_at)
    show_notes_tab
    expected = "by unknown #{I18n.l(@instance_note.reload.api_at, format: :default)}"
    assert_match(/#{Regexp.escape(expected)}/, response.body)
  end
