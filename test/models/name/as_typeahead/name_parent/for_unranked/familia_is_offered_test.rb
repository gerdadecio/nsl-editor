...
class NameParentFamiliaIsOfferedForUnrankedTest < ActiveSupport::TestCase
  test "name parent suggestion for unranked include familia" do
    typeahead = Name::AsTypeahead::ForParent.new(
      term: "a_family",
      avoid_id: 1,
      rank_id: NameRank.find_by(name: "[unranked]").id,
    )
    expected_ranks = ["Familia"]
