...
class NameParentSubformaIsOfferedForUnrankedTest < ActiveSupport::TestCase
  test "name parent suggestion for unranked include subforma" do
    typeahead = Name::AsTypeahead::ForParent.new(
      term: "a_subforma",
      avoid_id: 1,
      rank_id: NameRank.find_by(name: "[unranked]").id,
    )
    expected_ranks = ["Subforma"]
