...
class TypeaheadForSynonymySubfamiliaTest < ActiveSupport::TestCase
  def setup
    @ta = Instance::AsTypeahead::ForSynonymy.new("*",
                                                 names(:a_subfamilia).id)
  end

  test "instance typeahead for synonymy rank restriction for a subfamilia" do
  ...
  def check_exclusions
    %w[Regio Regnum Division Classis Subclassis Superordo Ordo Subordo Genus
       ...
       Subvarietas Forma Subforma Subfamilia].each do |rank_string|
